...
void aclk_create_header(BUFFER *dest, char *type, char *msg_id, time_t ts_secs, usec_t ts_us, char *rrdhost_guid)
{
    uuid_t uuid;
    char uuid_str[36 + 1];
    ...
    if (rrdhost_guid) {
        // optional field, if not set assume the message is related to
        // the host that sends the message (based on mqtt topic guid)
...
int aclk_send_single_chart(char *hostname, char *chart)
{
    ...
    aclk_create_header(local_buffer, "chart", msg_id, 0, 0, NULL);
    buffer_strcat(local_buffer, ",\n\t\"payload\": ");
