...
static void aclk_graceful_disconnect()
{
    ...
    aclk_create_header(b, "disconnect", NULL, 0, 0, aclk_shared_state.version_neg, localhost);
    buffer_strcat(b, ",\n\t\"payload\": \"graceful\"}\n");
...
static inline void aclk_hello_msg()
{
    ...
    aclk_create_header(buf, "hello", msg_id, 0, 0, ACLK_VERSION_NEG_VERSION, localhost);
...
inline void aclk_create_header(BUFFER *dest, char *type, char *msg_id, time_t ts_secs, usec_t ts_us, int version, RRDHOST *host)
{
    uuid_t uuid;
    char uuid_str[36 + 1];
    ...
    if (version >= ACLK_V_CHILD_HDR && host)
        buffer_sprintf(
            dest,
            ",\"mguid\": \"%s\""
            ",\"child\": \"%s\"",
...
void aclk_send_alarm_metadata(ACLK_METADATA_STATE metadata_submitted)
{
    ...
    if (metadata_submitted == ACLK_METADATA_SENT)
        aclk_create_header(local_buffer, "connect_alarms", msg_id, 0, 0, aclk_shared_state.version_neg, localhost);
    else
        aclk_create_header(local_buffer, "connect_alarms", msg_id, aclk_session_sec, aclk_session_us, aclk_shared_state.version_neg, localhost);
...
int aclk_send_info_metadata(ACLK_METADATA_STATE metadata_submitted, RRDHOST *host)
{
    ...
    // session.
    if (metadata_submitted == ACLK_METADATA_SENT)
        aclk_create_header(local_buffer, "update", msg_id, 0, 0, aclk_shared_state.version_neg, host);
    else
        aclk_create_header(local_buffer, "connect", msg_id, aclk_session_sec, aclk_session_us, aclk_shared_state.version_neg, host);
    buffer_strcat(local_buffer, ",\n\t\"payload\": ");
...
int aclk_send_info_child_connection(RRDHOST *host, ACLK_CMD cmd)
{
    ...
    aclk_create_header(local_buffer, cmd == ACLK_CMD_CHILD_CONNECT ? "child_connect" : "child_disconnect", msg_id, 0, 0, aclk_shared_state.version_neg, host);
...
int aclk_send_single_chart(RRDHOST *host, char *chart)
{
    ...
    aclk_create_header(local_buffer, "chart", msg_id, 0, 0, aclk_shared_state.version_neg, host);
    buffer_strcat(local_buffer, ",\n\t\"payload\": ");
...
int aclk_update_alarm(RRDHOST *host, ALARM_ENTRY *ae)
{
    ...
    buffer_flush(local_buffer);
    aclk_create_header(local_buffer, "status-change", msg_id, 0, 0, aclk_shared_state.version_neg, host);
    buffer_strcat(local_buffer, ",\n\t\"payload\": ");
